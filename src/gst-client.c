...
gboolean gstd_cli_parse_cmd (GstdCli* self, gchar** args, int args_length1, GError** error) {
	...
	if (_tmp37_ == ((0 != _tmp37__label0) ? _tmp37__label0 : (_tmp37__label0 = g_quark_from_static_string ("create")))) {
		...
		switch (0) {
			...
			{
				gboolean _tmp84_;
				_tmp84_ = gstd_cli_set_strict (self, args, args_length1);
				result = _tmp84_;
				return result;
			}
		}
			...
			default:
			{
				...
				if (args_length1 > 1) {
					...
					while (TRUE) {
						...
						_tmp86_ = strlen (self->priv->cmds[(id * self->priv->cmds_length2) + 0]);
						if (_tmp86_ < 6) {
							_tmp85_ = "\t\t";
						} else {
							_tmp85_ = "\t";
						}
						fprintf (stdout, " %s:%s%s\n", self->priv->cmds[(id * self->priv->cmds_length2) + 0], _tmp85_, self->priv->cmds[(id * self->priv->cmds_length2) + 2]);
						id++;
					}
					fprintf (stdout, "\n");
				}
