...
static void _dbus_pipeline_state_changed (GObject* _sender, DBusConnection* _connection);
static void _dbus_pipeline_error (GObject* _sender, DBusConnection* _connection);
...
gint64 pipeline_PipelineGetDuration (Pipeline* self) {
	...
	if (!gst_element_query_duration (self->priv->pipeline, &format, &duration)) {
		fprintf (stdout, "Unable to get duration to pipe\n");
		result = (gint64) (-1);
		return result;
	}
	result = duration;
...
gint64 pipeline_PipelineGetPosition (Pipeline* self) {
	...
	if (!gst_element_query_position (self->priv->pipeline, &format, &position)) {
		fprintf (stdout, "Unable to get position to pipe\n");
		result = (gint64) (-1);
		return result;
	}
	result = position;
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyInt (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (property);
	_tmp26_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT32, &_tmp26_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyString (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (property);
	_tmp32_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_STRING, &_tmp32_);
	_g_free0 (result);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
...
static DBusHandlerResult _dbus_pipeline_PipelineGetDuration (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp33_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT64, &_tmp33_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_PipelineGetPosition (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp34_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT64, &_tmp34_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static void pipeline_class_init (PipelineClass * klass) {
	...
	g_signal_new ("state_changed", TYPE_PIPELINE, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	g_signal_new ("error", TYPE_PIPELINE, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
