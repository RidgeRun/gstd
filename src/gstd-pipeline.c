/* gstd-pipeline.c generated by valac 0.15.0, the Vala compiler
...
static void gstd_pipeline_real_element_set_property_boolean_async (gstdPipelineInterface* base, const gchar* element, const gchar* property, gboolean val, GError** error);
...
static void _vala_gstd_pipeline_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec);
static void _vala_gstd_pipeline_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
...
guint gstd_pipeline_get_registration_id (gstdPipeline* self) {
	...
	_tmp0_ = self->priv->_registration_id;
	result = _tmp0_;
	return result;
}
...
void gstd_pipeline_set_registration_id (gstdPipeline* self, guint value) {
	...
	g_return_if_fail (self != NULL);
	_tmp0_ = value;
	self->priv->_registration_id = _tmp0_;
	g_object_notify ((GObject *) self, "registration-id");
}
...
static void gstd_pipeline_class_init (gstdPipelineClass * klass) {
	...
	G_OBJECT_CLASS (klass)->get_property = _vala_gstd_pipeline_get_property;
	G_OBJECT_CLASS (klass)->set_property = _vala_gstd_pipeline_set_property;
	G_OBJECT_CLASS (klass)->finalize = gstd_pipeline_finalize;
	/**
	   Set/get the dbus-path assigned when created
	 */
	g_object_class_install_property (G_OBJECT_CLASS (klass), GSTD_PIPELINE_PATH, g_param_spec_string ("path", "path", "path", NULL, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE | G_PARAM_WRITABLE));
	/**
	   Set/get the DBus registration id assigned when created
	 */
	g_object_class_install_property (G_OBJECT_CLASS (klass), GSTD_PIPELINE_REGISTRATION_ID, g_param_spec_uint ("registration-id", "registration-id", "registration-id", 0, G_MAXUINT, (guint) 0, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE | G_PARAM_WRITABLE));
}
...
static void gstd_pipeline_gstd_pipeline_interface_interface_init (gstdPipelineInterfaceIface * iface) {
	...
	iface->element_set_property_boolean_async = (void (*)(gstdPipelineInterface*, const gchar*, const gchar*, gboolean, GError**)) gstd_pipeline_real_element_set_property_boolean_async;
...
static void gstd_pipeline_instance_init (gstdPipeline * self) {
	...
	self->priv->_registration_id = (guint) 0;
}
...
static void _vala_gstd_pipeline_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec) {
	gstdPipeline * self;
	self = GSTD_PIPELINE (object);
	switch (property_id) {
		case GSTD_PIPELINE_PATH:
		g_value_set_string (value, gstd_pipeline_get_path (self));
		break;
		case GSTD_PIPELINE_REGISTRATION_ID:
		g_value_set_uint (value, gstd_pipeline_get_registration_id (self));
		break;
		default:
...
static void _vala_gstd_pipeline_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		case GSTD_PIPELINE_PATH:
		gstd_pipeline_set_path (self, g_value_get_string (value));
		break;
		case GSTD_PIPELINE_REGISTRATION_ID:
		gstd_pipeline_set_registration_id (self, g_value_get_uint (value));
		break;
		default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
