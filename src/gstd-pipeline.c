...
static gpointer __long_dup0 (gpointer self) {
...
gboolean pipeline_PipelineSkip (Pipeline* self, gint period_ms) {
	...
	cur_pos_ns = (gint64) 0;
	seek_ns = (gint64) 0;
	if (!gst_element_query_position (self->priv->pipeline, &format, &cur_pos_ns)) {
		result = FALSE;
		return result;
	}
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyLong (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (element);
	_g_free0 (property);
	dbus_message_iter_open_container (&iter, DBUS_TYPE_STRUCT, NULL, &_tmp37_);
	dbus_message_iter_close_container (&iter, &_tmp37_);
	_g_free0 (result);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyString (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (property);
	_tmp40_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_STRING, &_tmp40_);
	_g_free0 (result);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
...
static DBusHandlerResult _dbus_pipeline_PipelineSkip (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp46_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp46_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_PipelineSpeed (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp48_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp48_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
