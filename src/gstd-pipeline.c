/* gstd-pipeline.c generated by valac 0.12.1, the Vala compiler
...
gboolean pipeline_PipelineSetAutoStop (Pipeline* self, gint64 timeout_ms);
...
static gboolean pipeline_StartAutoStop (Pipeline* self);
static gpointer _pipeline_AutoStopThread_gthread_func (gpointer self);
...
static DBusHandlerResult _dbus_pipeline_PipelineSetAutoStop (Pipeline* self, DBusConnection* connection, DBusMessage* message);
static DBusHandlerResult _dbus_pipeline_PipelineAbortAutoStop (Pipeline* self, DBusConnection* connection, DBusMessage* message);
...
static DBusHandlerResult _dbus_pipeline_PipelineStep (Pipeline* self, DBusConnection* connection, DBusMessage* message);
static DBusHandlerResult _dbus_pipeline_PipelineSendCustomEvent (Pipeline* self, DBusConnection* connection, DBusMessage* message);
...
Pipeline* pipeline_construct (GType object_type, const gchar* description) {
	...
	_tmp3_ = gst_element_get_bus (self->priv->pipeline);
	bus = _tmp3_;
...
static GstBusSyncReply pipeline_bus_sync_callback (Pipeline* self, GstBus* bus, GstMessage* message) {
	...
	_tmp4_ = _gst_object_ref0 (GST_IS_PIPELINE (_tmp3_) ? ((GstPipeline*) _tmp3_) : NULL);
	pipe = _tmp4_;
	g_assert (pipe != NULL);
	_tmp5_ = gst_child_proxy_get_child_by_name ((GstChildProxy*) pipe, "videosink");
	_tmp6_ = _tmp5_;
	sink = GST_IS_ELEMENT (_tmp6_) ? ((GstElement*) _tmp6_) : NULL;
	if (sink == NULL) {
		result = GST_BUS_PASS;
	...
	_tmp7_ = sink;
	_tmp8_ = _gst_object_ref0 (GST_IS_X_OVERLAY (_tmp7_) ? ((GstXOverlay*) _tmp7_) : NULL);
	overlay = _tmp8_;
	if (overlay == NULL) {
		result = GST_BUS_PASS;
...
static gboolean pipeline_bus_callback (Pipeline* self, GstBus* bus, GstMessage* message) {
	...
	switch (message->type) {
		...
		case GST_MESSAGE_EOS:
		{
			...
			g_cond_signal (self->priv->autostop_eos_cond);
		...
		case GST_MESSAGE_QOS:
		{
			...
			guint64 _tmp22_;
			guint64 _tmp23_;
			gst_message_parse_qos (message, &_tmp12_, &_tmp13_, &_tmp14_, &_tmp15_, &_tmp16_);
			live = _tmp12_;
			running_time = _tmp13_;
			stream_time = _tmp14_;
			timestamp = _tmp15_;
			duration = _tmp16_;
			gst_message_parse_qos_values (message, &_tmp17_, &_tmp18_, &_tmp19_);
			jitter = _tmp17_;
			proportion = _tmp18_;
			quality = _tmp19_;
			gst_message_parse_qos_stats (message, &_tmp20_, &_tmp21_, &_tmp22_);
			fmt = _tmp20_;
			processed = _tmp21_;
			dropped = _tmp22_;
			format = (gint) fmt;
			_tmp23_ = pipeline_PipelineGetId (self);
			g_signal_emit_by_name (self, "qo-s", _tmp23_, live, running_time, stream_time, timestamp, duration, jitter, proportion, quality, format, processed, dropped);
			break;
		}
		default:
		{
...
gboolean pipeline_PipelineSetAutoStop (Pipeline* self, gint64 timeout_ms) {
	...
	if (!self->priv->autostop_thread_running) {
		self->priv->autostop_time_ms = timeout_ms;
		self->priv->trigger_autostop = TRUE;
	} else {
		syslog (LOG_ERR, "Can't set the autostop timer if the timer is already running", NULL);
		result = FALSE;
		return result;
	}
	result = TRUE;
...
static gpointer _pipeline_AutoStopThread_gthread_func (gpointer self) {
	gpointer result;
	result = pipeline_AutoStopThread (self);
	return result;
}
...
static gboolean pipeline_StartAutoStop (Pipeline* self) {
	...
	if (!_tmp0_) {
		syslog (LOG_DEBUG, "Cannot run without threads.\n", NULL);
		result = FALSE;
		return result;
	}
...
gboolean pipeline_PipelineSetState (Pipeline* self, gint state) {
	...
	if (_tmp1_) {
		_tmp0_ = self->priv->autostop_time_ms <= 0;
	} else {
		_tmp0_ = FALSE;
	}
	if (_tmp0_) {
		syslog (LOG_DEBUG, "Invalid time value. Timeout value must be > 0.\n", NULL);
		self->priv->trigger_autostop = FALSE;
		result = FALSE;
		return result;
	}
...
gboolean pipeline_ElementGetPropertyString (Pipeline* self, const gchar* element, const gchar* property, gchar** val) {
	...
	_tmp5_ = G_OBJECT_GET_CLASS ((GObject*) e);
	_tmp6_ = g_object_class_find_property (_tmp5_, property);
	_tmp7_ = _g_param_spec_ref0 (_tmp6_);
	spec = _tmp7_;
	if (spec == NULL) {
		syslog (LOG_WARNING, "Element %s does not have the property %s", element, property, NULL);
		result = FALSE;
...
gboolean pipeline_ElementGetPropertyBuffer (Pipeline* self, const gchar* element, const gchar* property, gchar** caps, guint8** data, int* data_length1) {
	...
	if (buffer != NULL) {
		...
		if (buffer->caps != NULL) {
			gchar* _tmp10_ = NULL;
			_tmp10_ = gst_caps_to_string (buffer->caps);
			_g_free0 (_tmp9_);
			_tmp9_ = _tmp10_;
		} else {
			gchar* _tmp11_;
		...
		_g_free0 (_caps);
		_caps = _tmp12_;
		_tmp13_ = (_tmp14_ = buffer->data, (_tmp14_ == NULL) ? ((gpointer) _tmp14_) : _vala_array_dup1 (_tmp14_, (gint) buffer->size));
		_data = (g_free (_data), NULL);
		_data = _tmp13_;
...
gboolean pipeline_PipelineSendCustomEvent (Pipeline* self, const gchar* stype, const gchar* name) {
	...
	GstEvent* _tmp4_ = NULL;
	g_return_val_if_fail (self != NULL, FALSE);
	g_return_val_if_fail (stype != NULL, FALSE);
	g_return_val_if_fail (name != NULL, FALSE);
	_tmp0_ = g_utf8_strdown (stype, (gssize) (-1));
	_tmp1_ = _tmp0_;
	_tmp2_ = (NULL == _tmp1_) ? 0 : g_quark_from_string (_tmp1_);
	g_free (_tmp1_);
	if (_tmp2_ == ((0 != _tmp2__label0) ? _tmp2__label0 : (_tmp2__label0 = g_quark_from_static_string ("upstream")))) {
		switch (0) {
			default:
			{
	...
	} else if (_tmp2_ == ((0 != _tmp2__label1) ? _tmp2__label1 : (_tmp2__label1 = g_quark_from_static_string ("downstream")))) {
		switch (0) {
			default:
			{
	...
	} else if (_tmp2_ == ((0 != _tmp2__label2) ? _tmp2__label2 : (_tmp2__label2 = g_quark_from_static_string ("downstream_oob")))) {
		switch (0) {
			default:
			{
	...
	} else if (_tmp2_ == ((0 != _tmp2__label3) ? _tmp2__label3 : (_tmp2__label3 = g_quark_from_static_string ("both")))) {
		switch (0) {
			default:
			{
	...
	} else if (_tmp2_ == ((0 != _tmp2__label4) ? _tmp2__label4 : (_tmp2__label4 = g_quark_from_static_string ("both_oob")))) {
		switch (0) {
			default:
			{
	...
	_tmp3_ = gst_structure_empty_new (name);
	_tmp4_ = gst_event_new_custom (type, _tmp3_);
	gst_element_send_event (self->priv->pipeline, _tmp4_);
	result = TRUE;
	return result;
}
...
static void pipeline_instance_init (Pipeline * self) {
	...
	self->priv->path = _tmp0_;
	self->priv->rate = 1.0;
	self->priv->autostop_time_ms = (gint64) 0;
	self->priv->autostop_thread_running = FALSE;
	self->priv->trigger_autostop = FALSE;
...
static DBusHandlerResult _dbus_pipeline_PipelineSetAutoStop (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_next (&iter);
	timeout_ms = _tmp1_;
	result = pipeline_PipelineSetAutoStop (self, timeout_ms);
	reply = dbus_message_new_method_return (message);
	dbus_message_iter_init_append (reply, &iter);
	_tmp2_ = result;
...
static DBusHandlerResult _dbus_pipeline_PipelineGetId (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp7_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_UINT64, &_tmp7_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_PipelineGetState (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp12_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT32, &_tmp12_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_ElementSetPropertyString (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (val);
	_tmp28_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp28_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyInt64 (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (property);
	_tmp39_ = val;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT64, &_tmp39_);
	_tmp40_ = result;
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyString (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (property);
	_tmp43_ = val;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_STRING, &_tmp43_);
	_g_free0 (val);
	_tmp44_ = result;
...
static DBusHandlerResult _dbus_pipeline_ElementGetState (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (element);
	_tmp46_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT32, &_tmp46_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_ElementGetPropertyBuffer (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_next (&iter);
	element = g_strdup (_tmp47_);
	dbus_message_iter_get_basic (&iter, &_tmp48_);
	dbus_message_iter_next (&iter);
	property = g_strdup (_tmp48_);
	data_length1 = 0;
	...
	_g_free0 (property);
	_tmp49_ = caps;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_STRING, &_tmp49_);
	_g_free0 (caps);
	_tmp50_ = data;
	dbus_message_iter_open_container (&iter, DBUS_TYPE_ARRAY, "y", &_tmp51_);
	for (_tmp52_ = 0; _tmp52_ < data_length1; _tmp52_++) {
		guint8 _tmp53_;
		_tmp53_ = *_tmp50_;
		dbus_message_iter_append_basic (&_tmp51_, DBUS_TYPE_BYTE, &_tmp53_);
		_tmp50_++;
	}
	dbus_message_iter_close_container (&iter, &_tmp51_);
	data = (g_free (data), NULL);
	_tmp54_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp54_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_PipelineGetPosition (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp56_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_INT64, &_tmp56_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_PipelineSendCustomEvent (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (name);
	_tmp67_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp67_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_ElementSetState (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	_g_free0 (element);
	_tmp70_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp70_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
static DBusHandlerResult _dbus_pipeline_Ping (Pipeline* self, DBusConnection* connection, DBusMessage* message) {
	...
	dbus_message_iter_init_append (reply, &iter);
	_tmp74_ = result;
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_BOOLEAN, &_tmp74_);
	if (reply) {
		dbus_connection_send (connection, reply, NULL);
		dbus_message_unref (reply);
		return DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult pipeline_dbus_message (DBusConnection* connection, DBusMessage* message, void* object) {
	...
	} else if (dbus_message_is_method_call (message, "com.ridgerun.gstreamer.gstd.PipelineInterface", "PipelineSetAutoStop")) {
		result = _dbus_pipeline_PipelineSetAutoStop (object, connection, message);
	} else if (dbus_message_is_method_call (message, "com.ridgerun.gstreamer.gstd.PipelineInterface", "PipelineAbortAutoStop")) {
		result = _dbus_pipeline_PipelineAbortAutoStop (object, connection, message);
	...
	} else if (dbus_message_is_method_call (message, "com.ridgerun.gstreamer.gstd.PipelineInterface", "PipelineStep")) {
		result = _dbus_pipeline_PipelineStep (object, connection, message);
	} else if (dbus_message_is_method_call (message, "com.ridgerun.gstreamer.gstd.PipelineInterface", "PipelineSendCustomEvent")) {
		result = _dbus_pipeline_PipelineSendCustomEvent (object, connection, message);
...
static void _dbus_pipeline_qo_s (GObject* _sender, guint64 pipe_id, gboolean live, guint64 running_time, guint64 stream_time, guint64 timestamp, guint64 duration, gint64 jitter, gdouble proportion, gint quality, gint format, guint64 processed, guint64 dropped, DBusConnection* _connection) {
	...
	dbus_int64_t _tmp88_;
	double _tmp89_;
	dbus_int32_t _tmp90_;
	dbus_int32_t _tmp91_;
	dbus_uint64_t _tmp92_;
	dbus_uint64_t _tmp93_;
	_path = g_object_get_data (_sender, "dbus_object_path");
	...
	_tmp88_ = jitter;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_INT64, &_tmp88_);
	_tmp89_ = proportion;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_DOUBLE, &_tmp89_);
	_tmp90_ = quality;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_INT32, &_tmp90_);
	_tmp91_ = format;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_INT32, &_tmp91_);
	_tmp92_ = processed;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_UINT64, &_tmp92_);
	_tmp93_ = dropped;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_UINT64, &_tmp93_);
	dbus_connection_send (_connection, _message, NULL);
	dbus_message_unref (_message);
}
