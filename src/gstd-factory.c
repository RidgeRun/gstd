/* gstd-factory.c generated by valac 0.12.1, the Vala compiler
...
Factory* factory_construct (GType object_type) {
	...
	self->priv->_pipes_size_ = FACTORY_num_pipes;
	{
		gint ids;
		ids = 0;
		{
...
gchar* factory_Create (Factory* self, const gchar* description) {
	...
	_tmp7_ = _tmp6_;
	_tmp8_ = g_strconcat ("/com/ridgerun/gstreamer/gstd/pipe", _tmp7_, NULL);
	_tmp9_ = _tmp8_;
	_g_free0 (_tmp7_);
	objectpath = _tmp9_;
		...
		index = 0;
		{
			...
			while (TRUE) {
				...
				if (self->priv->pipes[index] != NULL) {
					...
					_tmp2_ = _tmp1_;
					_tmp3_ = g_strcmp0 (_tmp2_, objectpath) == 0;
					_g_free0 (_tmp2_);
					if (_tmp3_) {
						Pipeline* _tmp4_;
						_tmp4_ = NULL;
