/* gstd-factory.c generated by valac 0.11.5, the Vala compiler
...
static DBusHandlerResult _dbus_factory_DestroyAll (Factory* self, DBusConnection* connection, DBusMessage* message);
...
Factory* factory_construct (GType object_type) {
	Factory * self = NULL;
	Pipeline** _tmp0_ = NULL;
	Pipeline** _tmp1_;
	self = (Factory*) g_object_new (object_type, NULL);
	_tmp0_ = g_new0 (Pipeline*, FACTORY_num_pipes + 1);
	_tmp1_ = _tmp0_;
	...
	self->priv->pipes = _tmp1_;
	{
		...
		ids = 0;
		{
			...
			while (TRUE) {
				...
				_tmp3_ = NULL;
				_g_object_unref0 (self->priv->pipes[ids]);
				self->priv->pipes[ids] = _tmp3_;
			}
		}
	}
	return self;
...
gchar* factory_Create (Factory* self, const gchar* description) {
	...
	_tmp3_ = pipeline_PipelineIsInitialized (self->priv->pipes[next_id]);
	if (!_tmp3_) {
		Pipeline* _tmp4_;
		gchar* _tmp5_;
		_tmp4_ = NULL;
		_g_object_unref0 (self->priv->pipes[next_id]);
		self->priv->pipes[next_id] = _tmp4_;
		_tmp5_ = g_strdup ("");
		result = _tmp5_;
		return result;
	}
	_tmp6_ = g_strdup_printf ("%i", next_id);
	_tmp7_ = _tmp6_;
	objectpath = (_tmp8_ = g_strconcat ("/com/ridgerun/gstreamer/gstd/pipe", _tmp7_, NULL), _g_free0 (_tmp7_), _tmp8_);
		...
		index = 0;
		{
			...
			while (TRUE) {
				...
				if (self->priv->pipes[index] != NULL) {
					...
					gchar* _tmp2_;
					gboolean _tmp3_;
					_tmp1_ = pipeline_PipelineGetPath (self->priv->pipes[index]);
					_tmp2_ = _tmp1_;
					if ((_tmp3_ = g_strcmp0 (_tmp2_, objectpath) == 0, _g_free0 (_tmp2_), _tmp3_)) {
						Pipeline* _tmp4_;
						_tmp4_ = NULL;
						_g_object_unref0 (self->priv->pipes[index]);
						self->priv->pipes[index] = _tmp4_;
						result = TRUE;
						return result;
					}
				}
	...
	_paths_size_ = 0;
	{
		...
		index = 0;
		{
			...
			while (TRUE) {
				...
				if (self->priv->pipes[index] != NULL) {
					gchar* _tmp2_ = NULL;
					_tmp2_ = pipeline_PipelineGetPath (self->priv->pipes[index]);
					_vala_array_add1 (&paths, &paths_length1, &_paths_size_, _tmp2_);
				}
			}
		}
	}
...
DBusHandlerResult factory_dbus_message (DBusConnection* connection, DBusMessage* message, void* object) {
	...
	} else if (dbus_message_is_method_call (message, "com.ridgerun.gstreamer.gstd.FactoryInterface", "DestroyAll")) {
		result = _dbus_factory_DestroyAll (object, connection, message);
...
GType factory_get_type (void) {
	...
	if (g_once_init_enter (&factory_type_id__volatile)) {
		...
		g_type_set_qdata (factory_type_id, g_quark_from_static_string ("DBusObjectVTable"), (void*) (&_factory_dbus_vtable));
